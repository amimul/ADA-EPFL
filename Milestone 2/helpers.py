...
from nltk.corpus import stopwords
weightIndicators=["ounce","teaspoon","tablespoon","cup","small","big","half"]
...
def matchTwoWordFrame(splitteds):
    ...
    while i<len(splitteds)-1:
        subMatches=get_close_matches(splitteds[i]+" "+splitteds[i+1],totalList,cutoff=0.85)
...
def matchOneWordFrame(splitteds):
    ...
    smallMatches={}
    for spls in splitteds:
        subMatches=get_close_matches(spls,totalList,cutoff=0.85)
        if len(subMatches)==0:
